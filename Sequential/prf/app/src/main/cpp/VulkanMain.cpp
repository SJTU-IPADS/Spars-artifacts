...
/*
 * sequential baseline with only relative infomation in each render node
 */
...
RenderNode *testRenderTree() {
    /* 根节点 */
    RenderNode *rootNode = new RenderNode(nullptr, 0, 0, 0, 1000, 2000, 0, 0);

    for(int i = 0; i < 200; i++) {
        /* 节点 */
        RenderNode *rectNode = new RenderNode(rootNode, i+1, rand()%900, rand()%1900, 300, 100, rand()%900, rand()%1900);
...
//    LOGI("%s", rootNode->dumpTree().c_str());
    ...
    CPU_SET(SEQ_BIND_CPU, &cpuset);  // 设置核心 ID，例如将线程绑定到核心 1

    int result = sched_setaffinity(0, sizeof(cpu_set_t), &cpuset);  // 设置亲和性
    if (result != 0) {
        LOGE("fail to set affinity %d!", SEQ_BIND_CPU);
    } else {
        LOGI("binds to core %d", SEQ_BIND_CPU);
    }
...
bool VulkanDrawFrame(android_app *app) {
    ...
    animationsList.animateAll(vSyncInfo); // 执行动画
    animationsList.updateTree(rootNode);  // 迭代整棵树修改子节点 // 所有节点都计算绝对位置
